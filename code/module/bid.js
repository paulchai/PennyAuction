...
exports.makeBought = function (req, mysql, q, bprice, discount, paid, bidcredits) {
    $mysqli = {};
    var id = req.param('id');
    ...
    if (req.param('ccode'))
        var escape_data = [id, req.session.userid, bprice, datge, req.param('ccode'), discount, paid, bidcredits];
    else
        var escape_data = [id, req.session.userid, bprice, datge, '', 0, paid, bidcredits];
...
exports.releaseAmount = function (req, mysql, q, bid, pid) {
    ...
    var escape_data = [bid.proposed_amount + pid.shipping_price, bid.proposed_amount + pid.shipping_price, bid.user_id]
    console.log(escape_data);
    query = mysql.query(strQuery, escape_data, defered.makeNodeResolver());
    return defered.promise;
};
...
exports.addBidInvoice = function (req, mysql, q) {
    ...
    payments.amount = global.general.eachbidpay;
    payments.insertInvoice();
};
...
/**
 * Returns list of bids when 0 is passed in as id and bid count when 1 is passed in as id
 * @param req
 * @param mysql
 * @param q
 * @param id 0|1
...
/**
 * Checks if the user has placed at least one bid to specific product
 * @param req
 * @param mysql
 * @param q
 * @returns {*}
...
exports.getBidsCredits = function (req, mysql, q) {
    $mysqli = {};
    strQuery = mysqli.mysqli($mysqli, 'bidcredits');
