...
{   var row = '';
    ...
    console.log(data);
	 ...
	 //if object is not an array convert it to array
	  if(Object.prototype.toString.call( data['shipping'] ) !== '[object Array]' &&  data['shipping']!='')	  {
		 var dummyarr = [];
		 dummyarr.push(data['shipping']);
		 data['shipping'] = dummyarr;
		}
		  
		 if(Object.prototype.toString.call( data['price'] ) !== '[object Array]' &&  data['price']!='' )	  {
		 var dummyarr = [];
		 dummyarr.push(data['price']);
		 data['price'] = dummyarr;
		  }
	 

  // generate mysql condition for price and shipping
	  if(data['shipping'].length > 0)
	  {
		 where +=' and ('; 
	  for(var i = 0;i<data['shipping'].length;i++)
	  {
  		   if(data['shipping'][i] == '>100')
		   where +='(shipping_price > 100)';
		   else if(	data['shipping'][i] == '-1')
		   where +='(shipping_price = 0)';
		   else
		   {
		   var to = parseFloat(data['shipping'][i])+50;
		   var from = parseFloat(data['shipping'][i]);
		   where +='(shipping_price BETWEEN '+from+' AND '+to+')';
		   }
		   
		    if(i!=data['shipping'].length-1)
		    where +=' OR ';
	  }
	      where +=')'; 
	  }
	 ...
	 if(data['price'].length > 0)
	  ...
	  for(var j = 0;j<data['price'].length;j++)
	  {
		   ...
		   {
		   var to = parseFloat(data['price'][j])+100;
		   var from = parseFloat(data['price'][j]);
		   where +='(bprice BETWEEN '+from+' AND '+to+')';
		   }
		   
		   if(j!=data['price'].length-1)
		   where +=' OR ';
	  }
	      where +=')'; 
	  }
...
exports.homeProducts = function(mysql,q,all,req,iscount)
{   
	...
	if(iscount == 1)
    $mysqli = {datge:datge,limit:'',field:'id'};
	else
	$mysqli = {datge:datge,limit:'limit '+page+',15',field:'*'};
...
exports.closeProducts = function (mysql,q,req,iscount)
{
	datge = dateFormat(new Date(),"yyyy-mm-dd HH:MM:ss");
    var page = (typeof(req.param('page')) !== 'undefined') ? req.param('page') : 0;
    page = (page > 0) ? (page-1)*10 : 0;
	...
	$mysqli = {datge:datge,limit:'limit '+page+',10',field:'*'};
	...
	for(i in results)
         {
			...
			if(results[i]['sprice'] != null || typeof(results[i]['sprice']) !== 'undefined')
			 ...
			 results[i]['pdescription'] = common.shorten(results[i]['description'],20); 
			}
