...
exports.addmessages = function (req, mysql, q) {
    ...
    customerio.sendEmail({
        ...
        content: {
            ...
            fname: req.session.first_name + ' ' + req.session.last_name,
            tname: req.body.first_name + ' ' + req.body.last_name,
...
exports.mymessages = function (req, mysql, q, count) {
    ...
    if (req.body.action == 'inbox') {
        ...
        $mysqli = {where: ' and p.to_status != "delete" and p.to_status != "archive"'};
        $mysqli['where1'] = 'and m.from_status != "delete" and m.from_status != "archive"';
    }
    var escape_data = "";
    if (count == 1) {
        strQuery = mysqli.mysqli($mysqli, 153);
        escape_data = [req.session.userid, req.session.userid];
    }
    else {
        strQuery = mysqli.mysqli($mysqli, 152);
        page = (req.body.page - 1) * 5;
        escape_data = [req.session.userid, req.session.userid, page];
        console.log(escape_data);
        //var escape_data =  [];
    }
    var defered = q.defer();
