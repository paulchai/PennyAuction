...
exports.doPayment = function (result) {  //console.log(self.gateway);
...
exports.insertInvoice = function () {
    ...
    if (self.type == 'winner' || self.type == 'bought') {
        ...
        customerio.createCustomer({
            email: self.sessionsdata.email,
            user: {firstName: self.sessionsdata.first_name, lastName: self.sessionsdata.last_name}
        });

        customerio.sendEmail({
            event: 'won_product',
            content: {
                user: self.sessionsdata.first_name + ' ' + self.sessionsdata.last_name,
                price: self.amount,
                title: self.sessionsdata.title,
                url: self.sessionsdata.url,
                email: self.sessionsdata.email
            }
        });
    }
    ...
    if (self.type == 'package') {
        ...
        customerio.sendEmail({
            event: 'package_bought',
            content: {user: self.sessionsdata.first_name + ' ' + self.sessionsdata.last_name, price: self.amount}
        });
    }

    if (self.type == 'Seat Reserved') {
        status = 'debit';
    }

    if (self.type == 'Seat Vacated') {
        status = 'credit';
    }
...
exports.setupPayment = function (result) {
    ...
    if (self.gateway == 'authorize')
        strQuery = mysqli.mysqli($mysqli, 'autorize27');
...
exports.payAuthorize = function (req, results) {
    ...
    if (self.gateway == 'stripe') {
        ...
        this.paymentid = (results.paymentid == '' || results.paymentid == 0) ? 0 : results.paymentid;
    }
    else if (self.gateway == 'authorize') {
        this.customerid = (results.acustomerid == '' || results.acustomerid == 0) ? results.id : results.acustomerid;
        this.paymentid = (results.apaymentid == '' || results.apaymentid == 0) ? 0 : results.apaymentid;
    }
    else {
...
exports.errorTrigger = function (error, page) {
    self.res.writeHead(302, {
        'Location': '/package/buy/' + self.payid + '/' + encodeURIComponent(error) + '?gateway=' + self.gateway
    });
    self.res.end();
    return false;
};
