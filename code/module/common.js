...
exports.processIndex = function(row,req,res,$arr)
{
  ...
  //console.log($arr.projects);
	...
	else  if($arr['pagetitle'] == 'Search Projects')
	{
		this.tplFile('search_results.tpl');
	    this.headerSet(1);
		this.loadTemplateHeader(req,res,$arr);
	}
    else
    {
...
exports.loadTemplateHeader = function(req,res,arr)
{
 ...
 q.all([this.productCategories(arr.config.mysql,q),user.userInfo(req,arr.config.mysql,q,uid,['balance','reserve_amount'])]).then(function(results){
  ...
  arr.pageofjs_Dash = originalurl;
  if(originalurl.indexOf("/messages") > -1)
  arr.pageofjs_Dash = '/dashboard/messages';
  if(originalurl.indexOf("/Address") > -1)
  arr.pageofjs_Dash = '/dashboard/Myaddress';
