...
//mysqli[5] = 'select c.id,c.parent_id,c.name, (COUNT(parent.id) - 1) AS depth from categories as c, categories AS parent where c.lft BETWEEN parent.lft AND parent.rgt  group by c.lft asc'; 
mysqli[5] = 'select c.id,c.name,c.parent_id, (COUNT(parent.id) - 1) AS depth,CONCAT(REPEAT(" - ", COUNT(parent.id) - 1),c.NAME) AS aligned_name from categories as c, categories AS parent where c.lft BETWEEN parent.lft AND parent.rgt  group by c.id   order by c.lft asc';
mysqli[6] = 'select p.* from projects AS p  where date_added <= "{{datge}}" and date_closed >= "{{datge}}" and buynow = 1 and market_status = "open" order by p.date_added desc';
mysqli[7] = 'select p.* from projects AS p inner join categories AS c on c.id = p.category_id,  categories AS parent where c.lft BETWEEN parent.lft AND parent.rgt and date_added <= "{{datge}}" and date_closed >= "{{datge}}" and market_status = "open"   {{cid}}  {{search}} {{where}} group by p.id {{orderby}} {{limit}}';
...
mysqli[13] = 'select p.* from projects AS p  inner join categories AS c on c.id = p.category_id,  categories AS parent where c.lft BETWEEN parent.lft AND parent.rgt and p.id > 0  and market_status = "open" {{cid}}  {{search}} group by p.id order by p.id desc';
mysqli[14] = 'insert into projects  (title,description,avatar,image,category_id,tags,user_id,shipping_price,shipping_description,created_at,buynow,feature,sprice,rprice,date_added,date_closed,status,bprice,mprice,iprice,wprice,auctionType,seats,seatAmount,storeid) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)';
mysqli[15] = 'select * from projects where user_id = ? and market_status !="removed" {{where}} order by id desc limit ? , 16';
...
mysqli[19] = 'select p.*,date_format(p.date_added,"%m/%d/%Y %H:%i:%s") as start,date_format(p.date_closed,"%m/%d/%Y %H:%i:%s") as close,date_format(p.date_added,"%m/%d/%Y") as date_add,date_format(p.date_closed,"%m/%d/%Y") as date_close,u.first_name,u.email,u.last_name from projects as p left join users as u on u.id = p.user_id where p.id = ? limit 1';
...
mysqli[24] = 'select b.user_id,SUBSTRING(CONCAT(u.first_name," ",upper(LEFT(u.last_name , 1))),1,10) AS name,b.proposed_amount from bids AS b inner join users as u on u.id = b.user_id where b.project_id =  ? order by proposed_amount DESC';
...
mysqli['autorize27'] = 'update users set acustomerid = ?, apaymentid = ?, paymethod = ? where id =  ? limit 1';
...
mysqli['buyerbidcount'] = 'SELECT SUM(`nobid`) as total_bid, SUM(`share`) as total_share FROM v_userbids WHERE user_id = ?';
mysqli['totalbidcount'] = 'SELECT SUM(`nobid`) as total_bid, SUM(`share`) as total_share FROM v_userbids';
mysqli['getmonthlyjackpot'] = 'SELECT bid_share, amount_total FROM `jackpot_monthly` WHERE (`date` = ((last_day(curdate()) + interval 1 day) - interval 1 month))';
mysqli['getdailybidamount'] = 'SELECT date, SUM(`total_amount`) as date_amount, SUM(`nobid`) as date_nobid,SUM(`share`) as date_share FROM `v_userbids` WHERE `date` = "{{date}}" GROUP BY `date`';
mysqli['getjackpotsum'] = 'SELECT DATE_FORMAT(date, "%Y-%m"), SUM(amount_bid) AS `bid_total`, SUM(bid_share) AS `share_total`, SUM(amount_mlm) AS `mlm_total` FROM jackpot_daily WHERE ((`date` >= ((last_day(curdate()) + interval 1 day) - interval 1 month)) AND (`date` < (last_day(curdate()) + interval 1 day))) GROUP BY DATE_FORMAT(date, "%Y-%m")';
mysqli['upsertjackpotdaily'] = 'INSERT INTO `jackpot_daily` (`date`, `amount_bid`, `bid_share`, `amount_mlm`, `create_at`, `update_at`) VALUES ("{{date}}", "{{amount_bid}}", "{{bid_share}}", "{{amount_mlm}}", CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) ON DUPLICATE KEY UPDATE amount_bid=VALUES(amount_bid), bid_share=VALUES(bid_share), amount_mlm=VALUES(amount_mlm), update_at=VALUES(update_at)';
mysqli['upsertjackpotMonthly'] = 'INSERT INTO `jackpot_monthly` (`date`, `amount_bid`, `bid_share`, `amount_mlm`, `amount_total`, `create_at`, `update_at`) VALUES (DATE_FORMAT("{{date}}" ,"%Y-%m-01"), "{{amount_bid}}", "{{bid_share}}", "{{amount_mlm}}", "{{amount_total}}", CURRENT_TIMESTAMP, CURRENT_TIMESTAMP) ON DUPLICATE KEY UPDATE amount_bid=VALUES(amount_bid), bid_share=VALUES(bid_share), amount_mlm=VALUES(amount_mlm), amount_total=VALUES(amount_total), update_at=VALUES(update_at)';
...
mysqli[64] = 'select email,first_name,last_name,balance,status,date_format(created_at,"%d %M, %Y") as cdate,id from users  order by created_at DESC limit ?, 10';
...
mysqli[73] = 'select p.*,date_format(p.date_added,"%m/%d/%Y") as date_add,date_format(p.date_closed,"%m/%d/%Y") as date_close,u.id AS user_id, u.email, CONCAT( UCASE(LEFT(u.first_name, 1)), SUBSTRING(u.first_name, 2)) AS first_name, CONCAT( UCASE(LEFT(u.last_name, 1)), SUBSTRING(u.last_name, 2)) AS last_name, CONCAT( UCASE(LEFT(u.address1, 1)), SUBSTRING(u.address1, 2)) AS address1, CONCAT( UCASE(LEFT(u.address2, 1)), SUBSTRING(u.address2, 2)) AS address2, CONCAT( UCASE(LEFT(u.country, 1)), SUBSTRING(u.country, 2)) AS country, CONCAT( UCASE(LEFT(u.state, 1)), SUBSTRING(u.state, 2)) AS state, CONCAT( UCASE(LEFT(u.city, 1)), SUBSTRING(u.city, 2)) AS city, u.zip, u.phone from projects as p LEFT JOIN users as u on u.id = (SELECT user_id FROM buynow WHERE project_id = p.id ) WHERE p.id > 0 {{where}}  order by p.id desc  limit ?, 10';
...
mysqli[80] = 'select i.*,date_format(i.date_added,"%m/%d/%Y") as date_add,u.first_name,u.email,u.last_name,p.title,p.id as pid,p.shipping_price from invoices as i left join projects as p on ((p.id = i.primary_id and i.type in ("sold","winner","holded","declined","Seat Reserved","Seat Vacated")) or (i.type = "bid" and p.id in (select b.project_id from bids as b where b.id = i.primary_id))) left join users as u on u.id = i.user_id where i.id > 0 {{where}} order by i.id desc  limit ?, 10';
mysqli[81] = 'select count(i.id) AS id,sum(i.amount) AS paid from invoices as i left join projects as p on ((p.id = i.primary_id and i.type in ("sold","winner","holded","declined","Seat Reserved","Seat Vacated")) or (i.type = "bid" and p.id in (select b.project_id from bids as b where b.id = i.primary_id))) left join users as u on u.id = i.user_id where i.id > 0 {{where}}';
mysqli[82] = 'select i.*,date_format(i.date_added,"%m/%d/%Y") as date_add,u.first_name,u.email,u.last_name,p.title,p.id as pid from invoices as i left join projects as p on ((p.id = i.primary_id and i.type in ("sold","winner","holded","declined","Seat Reserved","Seat Vacated")) or (i.type = "bid" and p.id in (select b.project_id from bids as b where b.id = i.primary_id))) left join users as u on u.id = i.user_id where i.id > 0 {{where}} order by i.id desc ';
mysqli[83] = 'insert into buynow (project_id,user_id,amount,date_added,couponcode,discount,paid,bidcredits) values(?,?,?,?,?,?,?,?)';
mysqli[84] = 'update users set balance=balance-? where id = ? limit 1';
mysqli[85] = 'select p.title,p.id,b.user_id,b.amount,p.status,p.avatar,date_format(b.date_added,"%d %M, %Y") AS date_add,b.paid,b.bidcredits,b.discount from buynow AS b inner join projects as p on p.id = b.project_id and b.user_id = ?  order by b.id desc limit ?, 5';
...
mysqli[97] = 'select p.*,u.first_name,u.last_name,SUBSTR(p.description,1,50) as description_short,date_format(p.date_added,"%m/%d/%Y") as date_add from blogs as p left join users as u on u.id = p.user_id   where p.id = ?';
...
mysqli[105] = 'select u.email,u.first_name,u.last_name,u.avatar,date_format(r.date_added,"%d %M, %Y") as added,r.id,r.status from referral as r left join users as u on r.to_id = u.id  where r.from_id = ?   order by r.id desc limit ? , 5';
mysqli[106] = 'select p.id,p.title from projects p where p.date_added <= "{{datge}}" and p.date_closed >= "{{datge}}" and p.market_status = "open" and p.status = 1  and p.id not in(select a.project_id from projects_autobid a where a.project_id = p.id and a.user_id =  ?  and a.status != "removed") order by p.title asc';
...
mysqli[151] = 'select p.*,SUBSTR(p.description,1,50) as description_short,date_format(p.date_added,"%m/%d/%Y") as date_add,u.first_name,u.email,u.last_name from blogs as p inner join users as u on u.id = p.user_id where p.id > 0  and p.status = "open"  and (p.title like "{{search}}%" or p.description like "{{search}}%") order by p.id desc  limit ?, 50';
...
mysqli[159] = 'SELECT id, email, CONCAT(UCASE(LEFT(first_name, 1)), SUBSTRING(first_name, 2)) AS first_name, last_name, admin FROM users ORDER BY first_name;';
...
mysqli[167] = 'select p.*,u.first_name,u.last_name,SUBSTR(p.description,1,50) as description_short,date_format(p.date_added,"%m/%d/%Y") as date_add from blogs as p left join users as u on u.id = p.user_id  where p.status="open"  order by p.id desc limit 5';
...
mysqli[170] = 'select * from withdrawals where user_id = ? order by id desc limit ? , 5';
...
mysqli[200] = 'select id,parent_id,lft,rgt from categories where parent_id = ? order by parent_id asc';
...
mysqli[133] = 'insert into coupons  (type,amount,code,date_added) values (?,?,?,?)';
mysqli[134] = 'select id,amount,type,code from coupons  where `code` = ? and status!="deleted" limit 1';
mysqli[135] = 'update coupons set  amount = ? , type = ?   where `id` = ? and status!="deleted" limit 1';
mysqli[136] = 'select {{field}} from coupons  where `code` != ""  and status!="deleted" order by id desc {{limit}}';
mysqli[137] = 'update coupons set  status = "deleted",date_updated = ?   where `code` = ? and status!="deleted" limit 1';
mysqli[138] = 'update coupons set  type = ?,amount = ?,date_updated = ?    where `code` = ? and status!="deleted" limit 1';
mysqli[139] = 'insert into product_shipping  (project_id,name,address,country,state,city,email,zipcode,phone,date_added,buyer_id) values (?,?,?,?,?,?,?,?,?,?,?)';
mysqli[140] = 'update projects set avatar = ? where id =  ? limit 1';
mysqli['bidcredits'] = 'select sum(i.amount) AS paid from invoices as i inner join bids as b on (b.id=i.primary_id) where i.type = "bid" and b.project_id =? and i.user_id = ? group by b.project_id';
mysqli[141] = 'update users set status = "active"  where id =  ? limit 1';
...
mysqli['getmlmuser'] = 'SELECT id,username,display_name,email,first_name,last_name,curr_rank,sp_no,date_format(created_at,"%m/%d/%Y %H:%i:%s"),balance,gender,address1,address2,zip,status FROM users WHERE role <> 0 and id = ?';
mysqli['findmlmuser'] = 'SELECT id,username,display_name,email,first_name,last_name,curr_rank,sp_no,date_format(created_at,"%m/%d/%Y %H:%i:%s"),balance,gender,address1,address2,zip,status FROM users WHERE role <> 0 and username = ?';
mysqli['createmlmusers'] = 'insert into users (username,display_name,email,role,first_name,last_name,password_hash,password_salt,curr_rank,sp_no,created_at,balance,gender,address1,address2,zip,status) values ("{{username}}","{{display_name}}","{{email}}",1,?,?,"{{password_hash}}","{{password_salt}}","{{curr_rank}}","{{sp_no}}","{{created_at}}","{{balance}}","{{gender}}","{{address1}}","{{address2}}","{{zip}}","active")';
mysqli['deletemlmusers'] = 'delete from users  where id = ?';
mysqli['updatemlmusers'] = 'update users set username = ?, display_name = ?, email = ?, first_name = ?, last_name = ?,password_hash = ?,password_salt = ?,curr_rank = ?,sp_no = ?,updated_at = ?,balance = ?,gender = ?, address1 = ?, address2 = ?,zip = ?, status = ?  where id =  ? limit 1';
...
mysqli[147] = 'select u.last_name from email_template as p left join users as u on u.id = p.user_id where p.id > 0   order by p.id desc';
mysqli[148] = 'select p.*,u.first_name,SUBSTR(p.message,1,50) as description_short,date_format(p.created_on,"%m/%d/%Y") as date_add from email_template as p left join users as u on u.id = p.user_id   where p.id = ?';
mysqli[149] = 'delete from email_template where id = ?';
mysqli[150] = 'select p.*,u.first_name,SUBSTR(p.message,1,50) as description_short,date_format(p.created_on,"%m/%d/%Y") as date_add from email_template as p left join users as u on u.id = p.user_id   where p.event = ?';
...
mysqli[266] = 'update projects  set status = "closed" where category_id in(select id from categories where id = ? or parent_id = ?';
mysqli[273] = 'select * from location where location_type = ? order by name asc';
mysqli[274] = 'delete from reviews where id = ?';
mysqli[275] = 'SELECT city.id, city.District FROM City WHERE city.CountryCode = ( SELECT CODE FROM Country WHERE Country.NAME = ? LIMIT 1) GROUP BY city.District ORDER BY city.District';

//stores feature
require('./stores_mysqli').include_stores(mysqli);
require('./seated_mysqli').include_seated(mysqli);
