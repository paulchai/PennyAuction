...
//emits = emitto;
	...
	var autobids = function(autobidData, emitto) {
		console.log(autobidData['pID']);
			...
			{
	//console.log(autobidData['wprice']);
	var aBid = [];
    ...
    else if(results[1][0][0]['wprice'] < wP)
    {
         ...
         q.all(bids.placeBid('',config.mysql,q,1,aBid)).then(function(resbid) {
          ...
          // console.log(2);
           ...
           customerio.createCustomer({email:results[2][0][0]['email'],user:{firstName:results[2][0][0]['first_name'],lastName:results[2][0][0]['last_name']}});
           customerio.sendEmail({event:'bid_submitted',content:{user:results[2][0][0]['first_name']+' '+results[2][0][0]['last_name'],title: prdt.title,amount : wP}});
         ...
         /*  res.writeHead(302, {
		   ...
		   var emitdata = {'pID':pID,'uID':uID,'wprice':wP,'winner':results[2][0][0]['first_name']+' '+results[2][0][0]['last_name'], 'date':prdt['date_closed'] };
