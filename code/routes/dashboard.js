...
app.post('/withdrawal', function(req, res){
  ...
  q.all([user.userInfo(req,config.mysql,q,req.session.userid,['balance','reserve_amount'])]).then(function(results){
         ...
         //console.log($arr['users']['reserve_amount']);   
...
app.get('/mybids', function(req, res){
  ...
  {     $arr['pagination'] = results[1][0].length;
        ...
        var paginator = new pagination.SearchPaginator({prelink:'/dashboard/mybids', current: req.body.page, rowsPerPage: 10, totalResult: $arr['pagination']});
...
app.get('/buynow', function(req, res){
  ...
  {     $arr['pagination'] = results[1][0].length;
        ...
        //console.log($arr['mywatchlist']);
        var pagination = require('pagination');
        var paginator = new pagination.SearchPaginator({prelink:'/dashboard/buynow', current: req.body.page, rowsPerPage: 10, totalResult: $arr['pagination']});
