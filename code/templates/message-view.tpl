        ...
        <div class="col-md-9">
            ...
            <div class="message_wb">
                    ...
                    <div class="medi-body">
                        <div class="mblue_txt">{$messages[0].subject} <span></span>
                            ...
                            <div class="pull-right">
                                {if ($messages[0].from_id == $loged.userid and $messages[0].from_status == 'archive') or
                                ($messages[0].to_id == $loged.userid and $messages[0].to_status == 'archive')} <a
                ...
                <div class="mwb_lbl padd20"> {foreach $messages as $key => $val}
                    ...
                    <div class="row"> {$val.fname}{if $val.read == 0 and $val.to_id == $loged.userid} <span
                            class="orange text text-danger">(New)</span>{/if}<br/>
                        {$val.message}<br/>
                        {$val.date_add}
                    </div>
                    {/foreach}

                    <form action="{$config.url}/dashboard/messages/save" method="POST" enctype="multipart/form-data"
                          class="admin_form" data-parsley-validate>
                        <input type="hidden" value="{$r_id}" name="r_id">

                        <div class="row">
                            <div class="col-md-12">
                                <textarea name="message" id="product_info" data-parsley-group="block1" required
                                          class="form-control" data-parsley-maxlength="500"
                                          data-parsley-length="[1, 500]"
                                          onkeyup="limitlengths(this, 500,'product_info_label')"></textarea>
                            </div>
                        </div>
                        <div class="row">
