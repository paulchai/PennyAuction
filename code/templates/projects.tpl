...
{if $proj|count <=  0}
<div class="today_head_lft">No Products Found</div>
{else}
{foreach $proj as $key => $val}
      <div class="slicebox">
        ...
        <div class="slicergt">
         <a href="{$config['url']}/product/view/{$val.id}" title="{$val.ptitle}"> <label> <a href="{$config['url']}/product/view/{$val.id}" title="{$val.ptitle}"> {$val.ptitle}  </a></label>
          <ul>
            <li> Bid to beat: </li>
